...
class Admin(commands.Cog):
  ...
  @commands.command(aliases=['Login', 'LOGIN'],
  ...
  async def login(self, ctx):
    ...
    if newmessagehash == hashedpass:
      ...
      em = discord.Embed(title="Logged in", colour=discord.Color.purple())
      em.set_footer(text=f"Requested by {ctx.author.name}",
                    icon_url=ctx.author.avatar_url)
      await ctx.send(embed=em)
      ...
      login_file.write(
        f"Login by {ctx.author.name} , in ,{ctx.message.created_at} \n")
      login_file.close()
  ...
  async def embed(self, ctx):
    embed = discord.Embed(
      title="Amir",
      url="https://github.com/amirdadfar9192",
      description=
    ...
    embed.set_thumbnail(
      url=
      'https://cdn.discordapp.com/avatars/365898254631567360/d325980732e089e58664f01c7174bac9.webp'
    )
  ...
  @commands.command(name='clear',
                    aliases=['c'],
                    help='Clears the current text channel')
  ...
  async def clear(self, ctx, amount: int = None):
    ...
    if amount is None:
      em = discord.Embed(
        title=
        "React to this message with :no_entry_sign: to confirm clearing the whole text channel. (This message automatically deletes after 5 seconds)",
        colour=discord.Color.purple())
      em.set_footer(text=f"Requested by {ctx.author.name}",
                    icon_url=ctx.author.avatar_url)
    ...
    if isinstance(amount, int) and amount > 0:
      ...
      em = discord.Embed(title=f"**{amount}** messages were deleted",
                         colour=discord.Color.purple())
      em.set_footer(text=f"Requested by {ctx.author.name}",
                    icon_url=ctx.author.avatar_url)
      ...
      em = discord.Embed(
        title=f"Please specify a valid number of messages to delete",
        colour=discord.Color.purple())
      em.set_footer(text=f"Requested by {ctx.author.name}",
                    icon_url=ctx.author.avatar_url)
