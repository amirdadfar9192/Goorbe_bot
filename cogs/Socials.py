...
class Socials(commands.Cog):
  ...
  async def what(self, ctx):
    await ctx.send(
      "https://cdn.discordapp.com/attachments/821739231096602689/917020202871435275/unknown.png"
    )

  @commands.command(name="end_cord", aliases=["endportal", "portal_cord"])
  async def end_cord(self, ctx):
    embed = discord.Embed(title="Cords:",
                          description='X:86 Y:58 Z:-3',
                          color=0xe9dc43)
    embed.set_thumbnail(
      url=
      'https://my.mcpedl.com/storage/texturepacks/775/images/nice-cats-texture-pack_3.png'
    )
  ...
  async def bye(self, ctx):
    await ctx.send(
      ' https://cdn.discordapp.com/attachments/662046048524435482/916652574294294578/unknown.png'
    )
...
#      await ctx.send('https://cdn.discordapp.com/attachments/821739231096602689/916966639994568764/unknown.png')
  ...
  async def random_joke(self, ctx):
    ...
    embed = discord.Embed(
      title="Joke",
      description="These jokes aren't even funny and i know it",
      color=0x02d475)
    embed.add_field(name="Setup",
                    value=json.loads(response.text)['setup'],
                    inline=True)
    embed.add_field(name="Delivery",
                    value=json.loads(response.text)['delivery'],
                    inline=True)
    await ctx.send(embed=embed)
    #print(joke_1)
...
# await ctx.voice_client.disconnect()
  ...
  async def echo(self, ctx):
    ...
    embed = discord.Embed(
      title="What Do You Want Me To Repeat??",
